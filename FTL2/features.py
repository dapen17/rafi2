...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^ftl hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl stop$'))
    ...
    # Tes koneksi bot
    @client.on(events.NewMessage(pattern=r'^ftl ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")

    # Broadcast pesan ke semua chat kecuali blacklist
    @client.on(events.NewMessage(pattern=r'^ftl bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. ftl hastle [pesan] [waktu][s/m/h/d]\n"
            ...
            "5. ftl bcstargr [waktu][s/m/h/d] [pesan]\n"
            "   Broadcast hanya ke grup dengan interval tertentu.\n"
            "6. ftl stopbcstargr[1-10]\n"
    ...
    @client.on(events.NewMessage(pattern=r'^ftl setreply'))
    ...
    @client.on(events.NewMessage(pattern=r'^ftl stopall$'))
